...
import { getNearestFile } from "../fs/mod.ts";
...
export const getPackageJson :() => Promise<PackageJson> = async () => {
    const packageJsonPath = getNearestFile('package.json');
    if (!packageJsonPath) {
        throw new Error('package.json not found');
    }
    const jsonBlob :PackageJson = await readJsonFile(packageJsonPath);
    return jsonBlob;
}

export const readJsonFile: <T>(filePath: string) => Promise<T> = async (
  filePath
) => {
...
export const command: (args: {
  ...
  cwd?: string;
  pipeToDeno?: boolean;
}) => Promise<CommandResult> = async (args) => {
...
export const npmPublish: (
  ...
  console.log("cwd", cwd);
  const packageJson: { version: string } = await readJsonFile(
...
export const npmVersion: (args: {
  cwd: string;
  npmVersionArg: string;
}) => Promise<CommandResult> = async (args) => {
  const { cwd, npmVersionArg } = args;
  const result = await command({
    cmd: ["npm", "version"].concat(npmVersionArg ? [npmVersionArg] : []),
    cwd,
  });
