...
import { existsSync, ensureDirSync } from "https://deno.land/std@0.130.0/fs/mod.ts";
...
const opts = getArgsFromEnvAndCli({
    // This env var is set to "true" by CI systems e.g. github actions
    // I don't think it can make sense to generate certs in those environments
    CI: false,
...
if (!existsSync(join(opts.CERTS_DIR, `${opts.APP_FQDN}-key.pem`))) {
    ensureDirSync(opts.CERTS_DIR);
    Deno.chdir(opts.CERTS_DIR);
    console.log(`mkcert -cert-file ${opts.APP_FQDN}.pem -key-file ${opts.APP_FQDN}-key.pem ${opts.APP_FQDN} localhost`)
    ...
    console.log(`👉 mkcert generated http certificates for ${opts.APP_FQDN} in ${opts.CERTS_DIR}`);
}
